...
export const CourierProfile: React.FC<CourierProfileProps> = ({ user, onSwitchRole, onLogout, onNavigate }) => {
  const targetRole = UserRole.SENDER;
  const switchLabel = 'Войти как Отправитель';
  const switchColor = '#0A84FF'; 
  ...
  return (
    ...
    >
        ...
        <div className="fixed inset-0 pointer-events-none -z-10">
             <div className="absolute top-[-10%] center w-[80%] h-[40%] bg-[#0A84FF]/20 blur-[100px] rounded-full opacity-60"></div>
        </div>
        ...
        <div className="space-y-6" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 20px)', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))' }}>
            ...
            <div className="flex flex-col items-center pt-8 pb-4 relative">
                ...
                >
                    ...
                    <div className="absolute bottom-1 right-1 z-20 bg-[#30D158] w-8 h-8 rounded-full flex items-center justify-center border-[4px] border-black text-black shadow-lg">
            ...
            <div>
                <ProfileBlock>
                    <MenuRow color="#0A84FF" icon={<Icons.User />} label="Личные данные" onClick={() => onNavigate && onNavigate('PROFILE_PERSONAL')} />
                    <MenuRow color="#BF5AF2" icon={<Icons.FileText />} label="Документы" subLabel="Проверено" isLast onClick={() => onNavigate && onNavigate('PROFILE_DOCS')} />
                </ProfileBlock>
            </div>
            ...
            <div>
                <ProfileBlock>
                    <MenuRow color="#30D158" icon={<Icons.Shield />} label="Безопасность" onClick={() => onNavigate && onNavigate('PROFILE_SECURITY')} />
                    <MenuRow color="#64D2FF" icon={<Icons.Message />} label="Поддержка" isLast onClick={() => onNavigate && onNavigate('PROFILE_SUPPORT')} />
                </ProfileBlock>
            </div>

            <div>
                <ProfileBlock>
                    <MenuRow color="#FF2D55" icon={<Icons.MapPin />} label="Ваш город" subLabel={user.city || 'Москва'} onClick={() => onNavigate && onNavigate('PROFILE_CITY')} />
                    <MenuRow color="#FF9F0A" icon={<Icons.Navigation />} label="Навигация" subLabel="Яндекс" isLast onClick={() => onNavigate && onNavigate('PROFILE_NAVIGATION')} />
                </ProfileBlock>
            </div>
