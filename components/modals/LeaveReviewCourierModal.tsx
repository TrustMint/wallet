...
interface LeaveReviewCourierModalProps {
    order: Order;
    onSubmit: (data: { rating: number; comment: string; tags: string[] }) => void;
    onClose: () => void;
}

// Keywords relevant for Courier reviewing Sender/Order
const POSITIVE_KEYWORDS = ["Вежливый", "Быстро встретил", "Точное описание", "Хорошая упаковка", "Удобный подъезд", "Чаевые"];
const NEGATIVE_KEYWORDS = ["Долго ждал", "Неверный адрес", "Груз тяжелее", "Грубил", "Не на связи", "Плохая упаковка"];

export const LeaveReviewCourierModal: React.FC<LeaveReviewCourierModalProps> = ({ order, onSubmit, onClose }) => {
    ...
    const handleSentiment = (type: 'positive' | 'negative') => {
        setSentiment(type);
        // Auto-set rating based on sentiment
    ...
    return (
        <div className="p-4 pt-0 pb-8">
            <div className="text-center mb-6 px-4">
                <h3 className="text-[22px] font-bold text-white tracking-tight mb-2">Оцените отправителя</h3>
                <p className="text-[15px] text-neutral-400">
                    Как прошел заказ <span className="text-white font-medium">"{order.title}"</span>?
                </p>
            </div>
